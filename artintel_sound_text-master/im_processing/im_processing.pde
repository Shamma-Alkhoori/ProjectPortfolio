...
public class Note {
  ...
  public Note(int index, char type, PVector position) {
    ...
    this.sound = new SoundFile(im_processing.this, "sounds/" + type + index + ".mp3");
  }
  ...
  public void display() {
    ...
    if (mousePressed && abs((this.position.x + this.noteWidth / 2) - mouseX) < (this.noteWidth / 2) && abs((this.position.y + this.noteHeight / (this.type == 'b' ? 2 : 1)) - mouseY) < (this.noteHeight / 2)) {
      this.playNote();
    }
  }
}
...
public class Midi {
  ...
  public String lyrics;      // song lyrics
  ...
  public Midi() {
    ...
    this.buttons[2] = new Button(new PVector(580, 150), color(48), "NEW LYRICS", false);
  ...
  public void getLyrics() throws Exception {
    ...
    String urlString = "http://20.106.252.218:8080/lyrics";
    ...
    StringBuilder output = new StringBuilder(result.toString());
    int c = 0;
    for (int i = 0; i < output.length(); i++) {
      if (c > 25 && output.charAt(i) == ' ') {
        output.setCharAt(i, '\n');
        c = 0;
  ...
  public void display() {
    ...
    fill(255);
    rect(30, 30, 190, 185, 4);
    fill(0);
    textAlign(LEFT);
    text(midi.lyrics, 42, 57);
    textAlign(CENTER);
  }
